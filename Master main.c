...
long acc_x, acc_y, acc_z, acc_total_vector, loop_timer;
int temperature;
...
for (int cal_int = 0; cal_int < 2000 ; cal_int ++) { //Run this code 2000 times
  if (cal_int % 125 == 0)digitalWrite(13, !digitalRead(13)); //Print a dot on the LCD every 125 readings
...
acc_total_vector = sqrt((acc_x * acc_x) + (acc_y * acc_y) + (acc_z * acc_z)); //Calculate the total accelerometer
vector
...
Wire.write(roll); // sends one byte
Wire.write(pitch); // sends one byte
Wire.write(yaw); // sends one byte
Wire.endTransmission(); // stop transmitting
while (micros() - loop_timer < 4000); //Wait until the loop_timer reaches 4000us (250Hz) before starting the next loop
if (micros()- loop_timer > 4000) digitalWrite(13,HIGH); // indication if the loop exceeds 4000us
else digitalWrite (13,LOW);
loop_timer = micros(); // calculate the constant loop time for integration
}
